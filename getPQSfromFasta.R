#Funcion para obtener las posiciones de los pqs a partir de un set de secuencias (.fasta)
#input: archivo fasta con las secuencias IGIs a evaluar
#output: lista conteniendo dos data frames:
#-tabla con el numero de pqs detectado en cada IGI
#-tabla con los detalles de posicion, hebra y score de cada pqs
library("pqsfinder")
getPQSfromFasta = function (fastaFile=NULL){
  ...
  #Bucleo cada secuencia, obtengo sus pqs y almaceno en dos tablas (numero de PQSs por secuencia IGI,
  #Detalles de PQSs)
  for (i in 1:length(IGIs)){
    print(paste0("Procesando IGI ",i," de ",length(IGIs)))
    #Realizo la busqueda de pqs
    pqsIGI = pqsfinder(IGIs[[i]])
    seq_id = names(IGIs[i])
    #Obtengo el numero de pqs obtenidos anteriormente
...
#Ejecuto la funcion (alrededor de 10 min si no hay problemas)
